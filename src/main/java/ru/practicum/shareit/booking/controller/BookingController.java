...
public class BookingController {
    ...
    @PostMapping
    public ResponseEntity<BookingOutDto> addBooking(
    ...
    public ResponseEntity<BookingOutDto> approveBooking(
            @RequestHeader(HEADER_USER) Long userId,
    ...
    @GetMapping
    public ResponseEntity<List<BookingOutDto>> getAllBookingsByBookerId(
            @RequestHeader(HEADER_USER) Long userId,
    ...
    @GetMapping("/owner")
    public ResponseEntity<List<BookingOutDto>> getAllBookingsForAllItemsByOwnerId(
            @RequestHeader(HEADER_USER) Long userId,
            @RequestParam(defaultValue = "ALL", required = false) String state,
            @RequestParam(defaultValue = "0", required = false) Integer from,
            @RequestParam(defaultValue = "10", required = false) Integer size
